...
public final class VideoPlaybackScreen extends Screen {
    ...
    private void handlePlaying() {
        ...
        if (!isFade) {
            if (nextBackend != null && nextBackend.isPlayerReady()) {
                promoteNextEntry();
                return;
            }
            waitingForNextTicks++;
    ...
    private void promoteNextEntry() {
        ...
        // Preloads sit paused at position 0 (setStartPaused). performSwap resumes
        // them when !fromCrossfade, so we can swap immediately without a seek-wait race.
    ...
    @Override
    public void removed() {
        // If this screen is replaced without going through onClose() (e.g. another screen
        // takes over), Minecraft only calls removed(). Release native resources and fire the
        // end event here too so WaterMedia backends never leak. The queue is intentionally NOT
        // resumed: something else is deliberately taking the screen.
        if (closed) {
            return;
        }
        finishPlayback();
